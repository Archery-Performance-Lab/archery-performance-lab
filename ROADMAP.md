...
- [ ] Shooting phase detection — `ShootingPhase` taxonomy (`types/phase.ts`) reviewed and corrected against real coaching methodology (Tommaso Franchini, FITARCO tessera 151218): Stance, PreDraw, Drawing, Anchor, Aiming, Expansion, Release, FollowThrough. `biomechanics/` (distance, angle, velocity over keypoints) and `shot-analysis/analyzeShotVideo()` (video file to pose sequence) exist as building blocks. Actual phase-boundary detection is not implemented: it needs numeric thresholds that can't be responsibly invented without real, labeled footage to calibrate against.
