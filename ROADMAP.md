...
- [ ] Technical performance metrics — `posture-analysis/analyzePosture()` computes six real-time postural checks (shoulder/hip level, both elbow angles, head tilt, torso verticality) from a single `PoseFrame`, plus `scripts/inspect-posture.cjs` to view them as a skeleton overlay on one real video frame (standalone `.html`, no new dependency). Deliberately ported from a real, working third-party reference tool ("Archery Posture Tracker", ghiggo.altervista.org/posture) rather than invented from scratch — its default ideal/warning ranges are a real starting point, not yet calibrated against any footage of Tommaso or validated by a coach. Still open, raised by Tommaso directly: the "Livello Avanzato per l'istruttore" manual's own alignment check (bow hand / draw elbow / head forming a triangle, plus a forearm-to-arrow line) needs a camera positioned directly above the archer, a different viewing angle from every calibration video collected so far — not reproduced by this module, and it is not yet known whether pose estimation even works reliably from that angle. Tommaso has an idea to bring to this before it's tackled.
