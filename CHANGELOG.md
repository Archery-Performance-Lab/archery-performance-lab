...
- Diagnosed the "known open question" the confidence-logging addition above was added to answer: it looked, from confidence scores alone, like it could be BlazePose confidently misplacing occluded draw-arm joints (0.99+ confidence, but positions that didn't match the photo). It wasn't — checked directly against the real video (not just the numbers) and found the default timestamp (`inspect-posture.cjs`'s "midpoint of video duration", picked with no knowledge of what that instant actually shows) landed *after* Release and FollowThrough had already finished, not during Anchor. The odd, tightly-bent elbow angle was an accurate reading of a hand already pulled back past the jaw, not a detection error. Documented in README.md, including the fix: use `scripts/inspect-elbow-angle.cjs`'s existing per-frame chart to find the real Anchor plateau by eye and pass that timestamp explicitly, rather than trusting the blind default.
