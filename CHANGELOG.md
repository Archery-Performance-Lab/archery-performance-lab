...
- A calibration-video requirement, found from a real run: future videos added to `raw-videos/` should specifically capture audible clicker sound. The slow-motion Kim Woojin Release clip (Berlin World Cup 2018) showed that the wrist-velocity fallback signal `phase-detection/` currently relies on can be too noisy to use even with the warmup-exclusion fix — with the draw-side hand close to the face for most of the clip, BlazePose's tracking got intermittently confused throughout, not just at the start, producing spurious velocity spikes with no clean ramp to detect. That video is kept as a visual reference only, not run through automated detection. `types/phase.ts` already documented the clicker's audio as the *real* detection signal (velocity was always meant as a fallback for when audio is unusable) — audio-based detection is not implemented yet, but this is now a documented requirement for footage collected going forward, in both `types/phase.ts` and `README.md`.
